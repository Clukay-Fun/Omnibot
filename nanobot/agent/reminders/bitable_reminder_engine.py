...
class BitableReminderRuleEngine:
    ...
    def load_rules(self) -> list[ReminderRule]:
        ...
        for index, item in enumerate(payload.get("tables", []), start=1):
            ...
            raw_cancel_on = item.get("cancel_on")
            ...
            when: dict[str, Any] = raw_when if isinstance(raw_when, dict) else {}
            schedule: dict[str, Any] = raw_schedule if isinstance(raw_schedule, dict) else {}
            cancel_on: dict[str, Any] = raw_cancel_on if isinstance(raw_cancel_on, dict) else {}
