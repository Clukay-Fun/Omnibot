...
- `doc create|read_text|append_text|create_blocks|trash`
...
支持标准 `docx`、`docs`、`wiki`、`file`、`drive/folder`、`base` URL 和对应原始 token。权限命令对原始 token 需要显式传 `--doc-type`；`public get|patch` 默认使用新版 `v2` 接口，也可通过 `--api-version v1` 切换到历史版本。`doc read_text` 返回纯文本提取结果，并对非文本块使用占位符；`append_text` 只追加纯文本段落；`create_blocks` 只允许向文档根块末尾追加一批平铺的基础块（文本、标题 1-3、无序/有序列表、引用）。
...
```bash
bash "{baseDir}/scripts/docs.sh" doc create_blocks \
  --document-id doccnxxxx \
  --children-json '[{"block_type":3,"heading1":{"elements":[{"text_run":{"content":"周报"}}]}},{"block_type":2,"text":{"elements":[{"text_run":{"content":"本周概览"}}]}}]'
```
...
- 不支持通用富文本 block 编辑；`doc create_blocks` 仅支持受控的根级基础块追加，不支持任意嵌套、任意位置插入、图片/表格/嵌入块写入。
