...
class _FakeClient:
    ...
    def send_message_sync(self, receive_id_type: str, receive_id: str, msg_type: str, content: str) -> bool:
        self.sent.append((receive_id_type, receive_id, msg_type, json.loads(content)))
        return True
...
async def test_outbound_messenger_sends_text_message() -> None:
    ...
    await messenger.send(OutboundMessage(channel="feishu", chat_id="ou_123", content="hello"))

    assert client.sent == [("open_id", "ou_123", "text", {"text": "hello"})]
...
async def test_outbound_messenger_sends_image_attachments(tmp_path) -> None:
    ...
    await messenger.send(OutboundMessage(channel="feishu", chat_id="oc_chat_1", content="", media=[str(image_path)]))

    assert client.uploaded_images == [str(image_path)]
    assert client.sent == [("chat_id", "oc_chat_1", "image", {"image_key": "img_key"})]
