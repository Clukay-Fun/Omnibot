...
def test_build_progress_renders_latest_six_entries_as_quotes() -> None:
    card = build_progress([f"entry-{i}" for i in range(8)])

    assert card["elements"][0]["content"] == "\n".join(
        f"> entry-{i}" for i in range(2, 8)
    )
...
def test_build_completed_appends_completion_marker() -> None:
    ...
    assert card["elements"][0]["content"].endswith("> 思考完成")
    assert "> 正在搜索网络：AI" in card["elements"][0]["content"]
